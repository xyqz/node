...
var Mina = function () {
	...
	_createClass(Mina, [{
		...
		value: function display() {
			var str = "";
			for (var i = 0; i < this.res.length; i++) {
				str += "\n\t\t\t\t\t\t\t\t\t\t<li>\n\t\t\t\t\t\t\t\t\t\t\t<h1>\n\t\t\t\t\t\t\t\t\t\t\t<img src=\"" + this.res[i].src + "\">\n\t\t\t\t\t\t\t\t\t\t\t</h1>\n\t\t\t\t\t\t\t\t\t\t\t<h2>\n\t\t\t\t\t\t\t\t\t\t\t\t<a href=\"#\" class=\"active\">" + this.res[i].link + "</a></br>\n\t\t\t\t\t\t\t\t\t\t\t\t<em>" + this.res[i].price + "</em></br>\n\t\t\t\t\t\t\t\t\t\t\t\t<del>" + this.res[i].del + "</del>\n\t\t\t\t\t\t\t\t\t\t\t</h2>\n\t\t\t\t\t\t\t\t\t\t\t<a href=\"#\" class=\"lianjie\">" + this.res[i].lianjie + "</a>\n\t\t\t\t\t\t\t\t\t\t</li>\n\t\t\t\t\t\t\t\t\t";
			}
			this.hot.html(str);
		}
	}]);
	...
	_createClass(Big, [{
		...
		value: function move() {
			...
			this.box.on("mousemove", function (eve) {
				...
				var l = e.offsetX - $(".img").children("span").get(0).offsetWidth / 2;
				var t = e.offsetY - $(".img").children("span").get(0).offsetHeight / 2;
				if (l < 0) l = 0;
				if (t < 0) t = 0;
				...
				var x = l / (that.box.get(0).offsetWidth - $(".img").children("span").get(0).offsetWidth);
				var y = t / (that.box.get(0).offsetHeight - $(".img").children("span").get(0).offsetHeight);
...
function _setCookie(key, value, day) {
	var d = new Date();
...
_require(["jq", "option", "inp"], function (_, option, inp) {
	new option.option();
	new inp.inp();
});(function ($) {
	"use strict";
		...
		banner: function banner(options) {
			...
			if (options.list != undefined && options.list.length > 0) {
				...
				this.LOCAL.listMove = function (i, type) {
					options.items.eq(that.LOCAL.index).css({ left: 0 }).stop().animate({
						left: -options.items.eq(0).width() * type
					}, that.LOCAL.moveTime).end().eq(i).css({ left: options.items.eq(0).width() * type }).stop().animate({
						left: 0
					}, that.LOCAL.moveTime);
				};
			...
			this.LOCAL.btnMove = function (type) {
				...
				if (that.LOCAL.listOnoff) {
					options.list.css({ background: "rgba(200,200,200,.5)" }).eq(that.LOCAL.index).css("background", "red");
				}
			};
...
$("#banner-move").banner({
	items: $("#banner-move").children(".img").children("img"),
	right: $("#banner-move").children("#right"),
	left: $("#banner-move").children("#left"),
	list: $("#banner-move").children(".list").children("span"),
	moveTime: 200,
	autoPlay: true,
	delayTime: 2000
});
...
$("#user").get(0).value = getCookie("user");
$("#pass").get(0).value = getCookie("pass");

$("#btn").on("click", function () {
	if ($("#ck").get(0).checked) {
		_setCookie("user", $("#user").val(), 7);
		_setCookie("pass", $("#pass").val(), 7);
	} else {
		_setCookie("user", $("#user").val());
		_setCookie("pass", $("#pass").val());
	}
});
...
var Ad = function () {
	...
	_createClass(Ad, [{
		...
		value: function display() {
			...
			var str2 = "";
			for (var i = 0; i < this.res.length; i++) {
				if (this.promise[this.promise.length - 1].id == this.res[i].ID) {
					str += "<img src=\"" + this.res[i].src + "\" > \n\t\t\t\t\t\t\t<p></p>\n\t\t\t\t\t\t\t<span></span>";

					str2 += "<img src=\"" + this.res[i].src + "\" >";
				}
			}
			this.img.html(str);
			$(".big").html(str2);
